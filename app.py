...
- Penawaran ini berlaku 14 hari setelah dibuat."""
    ...
    from xlsx_maker import generate_quotation_xlsx

    pdf_file  = generate_quotation_pdf(st.session_state["keranjang_proyek"], profit_analysis, client_data)
    xlsx_file = generate_quotation_xlsx(st.session_state["keranjang_proyek"], profit_analysis, client_data)
    ...
    with col_xlsx:
        st.download_button(
            label="📊 DOWNLOAD INVOICE PENAWARAN (XLSX)",
            data=xlsx_file,
            file_name=f"Penawaran_Angkasa_{nama_klien}.xlsx",
